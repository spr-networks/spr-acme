...
ARG LEGO_COMMIT=6ae6f51eed37ccd4e16e597e60be9c2dc206b61d
...
RUN set -eux; \
    git init -q lego; \
    cd lego; \
    git remote add origin https://github.com/go-acme/lego.git; \
...
COPY --from=builder /lego /usr/local/bin/lego
COPY --from=builder /spr_acme_plugin /spr_acme_plugin
